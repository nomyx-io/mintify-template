...
const NftRecordDetail = ({
  ...
  const tokenDetails = (
    <Card className="bg-nomyx-dark2-light dark:bg-nomyx-dark2-dark border-nomyx-gray4-light dark:border-nomyx-gray4-dark text-nomyx-text-light dark:text-nomyx-text-dark">
  ...
  return (
    <>
      ...
      >
        <div className="grid grid-cols-3 gap-4">
          {topFields.map((field: NftRecordDetailField, index: number) => (
            <div
              key={`field-${index}`}
              className="grid-cols-1 overflow-hidden text-ellipsis font-[1.3em]"
            >
              <p className="text-xs">
      ...
      <Card title="About Kronos Tokens" className="bg-nomyx-dark2-light dark:bg-nomyx-dark2-dark border-nomyx-gray4-light dark:border-nomyx-gray4-dark text-nomyx-text-light dark:text-nomyx-text-dark">
        <p>Kronos Yield Generating NBT</p>
        <p className="text-[#871212]">{`This ra-NBTI asset (Note-Backed Token) entitles you to participation rights in Loan#${
          data?.loanId || ""
        } with an origination value of $${
          data?.loanAmount || ""
        } with lender qualifier of ${
          data?.ficoScore || ""
        } FICO ranking. generating an annual yield ${
          data?.yield || ""
        }.It provides a total payout of $${
          data?.loanAmount || ""
        } over a term of ${data?.term || ""} months.`}</p>
      </Card>
